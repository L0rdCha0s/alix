...
void IdentifyVersion (void)
{
    ...
    if (p && p < myargc - 1)
    {
        iwad_path = myargv[p + 1];
    }

    snprintf(basedefault, sizeof(basedefault), "%s", "doom.cfg");

    serial_printf("[doom] IdentifyVersion iwad=%s\r\n", iwad_path);
    int access_res = access(iwad_path, R_OK);
    ...
    if (access_res == 0)
    {
        ...
        uint64_t directory_bytes = (uint64_t)numlumps * (uint64_t)sizeof(filelump_t);
        uint64_t infotableofs_u = (uint64_t)infotableofs;
        if (infotableofs_u > st.st_size || st.st_size - infotableofs_u < directory_bytes)
        {
            close(handle);
            I_Error("IdentifyVersion: invalid WAD directory offset %d", infotableofs);
        }
    ...
    I_Error("IdentifyVersion: unable to read IWAD at %s", iwad_path);
}
...
void D_DoomMain (void)
{
    ...
    serial_printf("[doom] D_DoomMain start\r\n");
    FindResponseFile ();
	
    IdentifyVersion ();
    serial_printf("[doom] version identified\r\n");
...
*/
    ...
    V_Init ();
    serial_printf("[doom] V_Init done\r\n");

    printf ("M_LoadDefaults: Load system defaults.\n");
    M_LoadDefaults ();              // load before initing other systems
    serial_printf("[doom] M_LoadDefaults done\r\n");

    printf ("Z_Init: Init zone memory allocation daemon. \n");
    Z_Init ();
    serial_printf("[doom] Z_Init done\r\n");

    printf ("W_Init: Init WADfiles.\n");
    W_InitMultipleFiles (wadfiles);
    serial_printf("[doom] W_InitMultipleFiles done\r\n");
