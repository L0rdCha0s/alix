...
static const uint8_t font8x8_basic_data[FONT_BASIC_CHAR_COUNT][FONT_BASIC_HEIGHT] = {
    ...
    {0x7C,0xC6,0x0E,0x3C,0x70,0xC0,0xFE,0x00}, /* '2' */
    ...
    {0xC6,0x66,0x3C,0x18,0x3C,0x66,0xC6,0x00}, /* 'X' */
    ...
    {0xE0,0x60,0x7C,0x66,0x66,0x66,0xE6,0x00}, /* 'h' */
    ...
    {0x7C,0xC6,0xDE,0xF6,0xC6,0xC6,0x7C,0x00}  /* DEL placeholder */
};

const uint8_t *font_basic_get_glyph8x8(uint8_t ch)
{
    if (ch < FONT_BASIC_FIRST_CHAR || ch > FONT_BASIC_LAST_CHAR)
    {
        return NULL;
    }
    return font8x8_basic_data[ch - FONT_BASIC_FIRST_CHAR];
}
...
void font_basic_copy_glyph8x16(uint8_t ch, uint8_t dest[FONT_BASIC_HEIGHT_X2])
{
    ...
    if (!glyph)
    {
        memset(dest, 0, FONT_BASIC_HEIGHT_X2);
        return;
    }
    for (int row = 0; row < FONT_BASIC_HEIGHT_X2; ++row)
    {
        dest[row] = glyph[row / 2];
    }
}
