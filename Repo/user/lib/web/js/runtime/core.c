...
js_runtime_t *js_runtime_create(void)
{
    ...
    rt->programs = NULL;
    rt->native_meta = NULL;
    if (!js_runtime_register_native(rt, "Number", js_builtin_number, NULL, true))
    {
        js_runtime_destroy(rt);
        return NULL;
    }
    if (!js_runtime_register_native(rt, "TypeError", js_builtin_type_error, NULL, true))
    {
        js_runtime_destroy(rt);
        return NULL;
    }
    if (!js_runtime_register_native(rt, "escape", js_builtin_escape, NULL, false))
    {
        js_runtime_destroy(rt);
        return NULL;
    }
...
{
    return js_runtime_register_native(rt, name, fn, user_data, false);
}
