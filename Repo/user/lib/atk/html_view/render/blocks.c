                                            ...
                                            html_view_margin_state_t *out_prev_pending)
{
    ...
    if (ctx->suppress_margin_top_node == node)
    {
        margin_top = 0;
        ctx->suppress_margin_top_node = NULL;
    }
                                             ...
                                             int *out_h)
{
    ...
    html_view_perf_note_measure(ctx->priv, HTML_VIEW_TRACE_MEASURE_BLOCK);
...
bool html_view_render_block_element(html_view_ctx_t *ctx, const html_node_t *node, const css_style_t *style)
{
    ...
    if (strcmp(tag, "h1") == 0)
    {
        ...
        (void)html_view_apply_block_margin_top(ctx, node,
                                               margin_top,
    ...
    if (border_top == 0 &&
        pad_top == 0 &&
        !(style->has_overflow && style->overflow != CSS_OVERFLOW_VISIBLE) &&
        !(style->has_display && style->display == CSS_DISPLAY_TABLE))
    {
        const html_node_t *collapse_child = NULL;
        int child_margin_top = 0;
        for (const html_node_t *child = node->first_child; child; child = child->next_sibling)
        {
            ...
            bool positioned = child_style.has_position &&
                              (child_style.position == CSS_POSITION_ABSOLUTE ||
                               child_style.position == CSS_POSITION_FIXED);
            bool floated = child_style.has_float && child_style.float_mode != CSS_FLOAT_NONE;
            if (!block_child || positioned || floated)
            {
                css_style_release(&child_style);
                continue;
            }

            if (child_style.has_margin && child_style.margin.top.valid && !child_style.margin.top.is_auto)
            {
                int child_font_px = html_view_font_px_for_style(ctx, &child_style, ctx->base_font_px);
                if (child_font_px <= 0)
                {
                    child_font_px = ctx->base_font_px;
                }
                child_margin_top = html_view_length_to_px_signed(&child_style.margin.top,
                                                                 ctx->viewport_w,
                                                                 ctx->viewport_h,
                                                                 content_w,
                                                                 ctx->viewport_h,
        ...
        if (collapse_child)
        {
            html_view_margin_state_t collapsed = html_view_margin_state_from_value(margin_top);
            html_view_margin_state_t child_state = html_view_margin_state_from_value(child_margin_top);
            html_view_margin_state_merge(&collapsed, &child_state);
            margin_top = html_view_margin_state_value(&collapsed);
            ctx->suppress_margin_top_node = collapse_child;
        }
    }
    ...
    bool sticky = style->has_position && style->position == CSS_POSITION_STICKY;
    const char *scroller_anchor = NULL;
    bool scroller_affix = false;
    if (!sticky)
    {
        scroller_anchor = html_view_scroller_anchor_for_node(node);
        if (scroller_anchor)
        {
            sticky = true;
            scroller_affix = true;
    ...
    int sticky_container_top = 0;
    int sticky_container_bottom = 0;
    ...
    if (sticky)
    {
        if (style->has_top && style->top.valid && !style->top.is_auto)
        {
            ...
            sticky_top_px = html_view_length_to_px_signed(&style->top,
                                                          ctx->viewport_w,
                                                          ctx->viewport_h,
                                                          ctx->body_w,
                                                          ctx->viewport_h,
        ...
        if (style->has_bottom && style->bottom.valid && !style->bottom.is_auto)
        {
            sticky_flags |= HTML_VIEW_STICKY_BOTTOM;
            sticky_bottom_px = html_view_length_to_px_signed(&style->bottom,
                                                             ctx->viewport_w,
                                                             ctx->viewport_h,
                                                             ctx->body_w,
                                                             ctx->viewport_h,
        ...
        if (sticky_flags != 0)
        {
            sticky_origin_y = border_doc_y - ctx->doc_origin_y;
            if (scroller_affix && scroller_anchor && ctx->priv)
            {
                int anchor_y = 0;
            ...
            sticky_container_top = ctx->pos_y - ctx->doc_origin_y;
            sticky_container_bottom = sticky_container_top + ctx->pos_h;
            if (ctx->record && ctx->priv)
            {
    ...
    bool defer_bg = false;
    bool defer_border = false;
    size_t deferred_bg_op = (size_t)-1;
    if (ctx->record &&
        !height_specified &&
        (style->has_background || style->has_border) &&
        !(style->has_background_image && style->background_image))
    {
        defer_border = style->has_border;
        defer_bg = style->has_background && !style->background_transparent;
        defer_box = defer_border || defer_bg;
        if (defer_bg && border_box_w > 0)
        {
            int draw_y = html_view_draw_y(ctx, draw_border_y);
            deferred_bg_op = html_view_record_rect_placeholder(ctx,
                                                               draw_border_x,
                                                               draw_y,
                                                               border_box_w,
                                                               0,
    ...
    int saved_table_row_height = ctx->table_row_height;
    bool saved_table_row_height_valid = ctx->table_row_height_valid;
    if (table_layout)
    {
        int table_w = 0;
        int table_h = 0;
        html_view_measure_css_table_cells(ctx, node, style, content_w, ctx->viewport_h, &table_w, &table_h);
        ctx->table_row_height = table_h;
        ctx->table_row_height_valid = table_h > 0;
    ...
    if (table_layout)
    {
        ...
        ctx->table_row_height = saved_table_row_height;
        ctx->table_row_height_valid = saved_table_row_height_valid;
    }
    if (clip_applied)
    {
        ctx->clip = saved_clip;
    ...
    if (defer_box)
    {
        ...
        int draw_y = html_view_draw_y(ctx, draw_border_y);
        if (defer_bg &&
            deferred_bg_op != (size_t)-1 &&
            ctx->priv &&
            deferred_bg_op < ctx->priv->render_cache.op_count)
        {
            html_view_op_t *op = &ctx->priv->render_cache.ops[deferred_bg_op];
            op->w = border_box_w;
            op->h = border_box_h;
            op->y = html_view_record_y(ctx, draw_y);
        }
        if (defer_border && border_box_w > 0 && border_box_h > 0)
        {
            if (border_top > 0 || border_right > 0 || border_bottom > 0 || border_left > 0)
            {
                html_view_draw_border_sides_clipped(ctx,
    ...
    if (sticky_op_start != (size_t)-1 && ctx->priv && sticky_flags != 0)
    {
        int sticky_box_h = content_h + pad_top + pad_bottom + border_top + border_bottom;
        size_t sticky_op_end = ctx->priv->render_cache.op_count;
