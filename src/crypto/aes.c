...
static void add_round_key(uint8_t state[16], const uint32_t *rk)
{
    ...
    {
        uint32_t k = rk[i];
        state[i * 4 + 0] ^= (uint8_t)(k >> 24);
        state[i * 4 + 1] ^= (uint8_t)(k >> 16);
        state[i * 4 + 2] ^= (uint8_t)(k >> 8);
        state[i * 4 + 3] ^= (uint8_t)(k);
    }
}
...
void aes128_encrypt_block(const aes128_enc_ctx_t *ctx, uint8_t block[16])
{
    ...
    for (round = 1; round < rounds; ++round)
    {
        ...
        shift_rows(block);
        mix_columns(block);
        add_round_key(block, ctx->round_keys + round * 4);
    }
    sub_bytes(block);
    shift_rows(block);
    add_round_key(block, ctx->round_keys + rounds * 4);
}
...
static void inv_mix_single(uint8_t col[4])
{
    uint8_t a = col[0];
    uint8_t b = col[1];
    uint8_t c = col[2];
...
{
    aes128_enc_ctx_t enc;
    aes128_init_encrypt(&enc, key);
    for (int i = 0; i < 44; ++i)
    {
