...
static __attribute__((aligned(16))) uint8_t g_rx_buffer[RTL_RX_BUFFER_SIZE];
...
static void rtl8139_handle_receive(void)
{
    while ((inb(g_io_base + RTL_REG_CR) & RTL_CR_RX_EMPTY) == 0)
    {
        ...
        while (next >= RTL_RX_BUFFER_SIZE)
        {
            next -= RTL_RX_BUFFER_SIZE;
        }
        g_rx_offset = next;
...
static uint16_t rtl8139_buffer_read16(uint32_t offset)
{
    uint32_t index0 = offset % RTL_RX_BUFFER_SIZE;
    uint32_t index1 = (index0 + 1) % RTL_RX_BUFFER_SIZE;
    return (uint16_t)(g_rx_buffer[index0] | ((uint16_t)g_rx_buffer[index1] << 8));
}

static uint8_t rtl8139_buffer_read8(uint32_t offset)
{
    return g_rx_buffer[offset % RTL_RX_BUFFER_SIZE];
}
