...
extern uint32_t g_sched_paging_lock_log_enable;
extern uint32_t g_sched_memcpy_log_enable;
...
static inline bool sched_paging_lock_log_enabled(void)
{
    return __atomic_load_n(&g_sched_paging_lock_log_enable, __ATOMIC_ACQUIRE) != 0;
}

static inline bool sched_memcpy_log_enabled(void)
{
    return __atomic_load_n(&g_sched_memcpy_log_enable, __ATOMIC_ACQUIRE) != 0;
}
...
#define SCHED_MEMCPY_LOG(...) do { if (sched_memcpy_log_enabled()) { serial_printf(__VA_ARGS__); } } while (0)
